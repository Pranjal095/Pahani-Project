      ...
      <div className="bg-white rounded-xl shadow-lg border">
        <div className="p-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Administrative Dashboard</h1>
              <p className="text-slate-600 mt-2">Manage and process Pahani document requests</p>
            </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-2xl font-bold text-slate-900">
                {requests.length > 0 ? Math.round((processedCount / requests.length) * 100) : 0}%
              </p>
        ...
        {loading ? (
          <div className="flex justify-center items-center py-24">
            <div className="text-center">
              ...
              <p className="text-slate-600 font-medium">Loading request data...</p>
            </div>
          </div>
        ) : error ? (
          <div className="p-8">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-red-800">
                    Unable to retrieve request data. Please check your connection and try refreshing the page.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  {filteredRequests.map((request) => (
                    ...
                    >
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-3">
                            <h3 className="text-lg font-semibold text-slate-900">
                              {request.district} → {request.mandal} → {request.village}
                            </h3>
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                              ...
                              {request.processed ? "Completed" : "Pending Review"}
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div>
                              ...
                              <p className="text-slate-600">{request.from_date} to {request.to_date}</p>
                            </div>
                            <div>
                              <span className="font-medium text-slate-700">Request ID:</span>
                              <p className="text-slate-600 font-mono">#{request.id}</p>
                            </div>
                            <div>
                              <span className="font-medium text-slate-700">Submitted:</span>
                              <p className="text-slate-600">{formatDate(request.timestamp)}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex-shrink-0">
                          <button
                            disabled={request.processed || processingId === request.id}
